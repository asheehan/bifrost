defmodule Bifrost.EventsTest do
  ...
  describe "events" do
    ...
    @valid_attrs %{name: "Summer Wedding 2024"}
    @update_attrs %{name: "Summer Wedding 2024 - Updated"}
    @invalid_attrs %{name: nil, user_id: nil}

    test "list_events/0 returns all events", %{user: user} do
      event = event_fixture(user)
      assert Events.list_events() == [event]
    end

    test "list_events_for_user/1 returns events for a specific user", %{user: user} do
      user2 = user_fixture()
      event1 = event_fixture(user)
      event2 = event_fixture(user2)

      user_1_events = Events.list_events_for_user(user.id)
      assert length(user_1_events) == 1
      assert hd(user_1_events).id == event1.id
      refute Enum.any?(user_1_events, fn e -> e.id == event2.id end)
    end

    test "get_event!/1 returns the event with given id", %{user: user} do
      event = event_fixture(user)
      assert Events.get_event!(event.id).id == event.id
    end

    test "get_event_by_slug!/1 returns the event with given slug", %{user: user} do
      event = event_fixture(user)
      assert Events.get_event_by_slug!(event.slug).id == event.id
    end

    test "create_event/1 with valid data creates an event", %{user: user} do
      assert {:ok, %Event{} = event} =
               Events.create_event(Map.put(@valid_attrs, :user_id, user.id))
    ...
    test "create_event/1 automatically generates a UUID slug", %{user: user} do
      attrs = Map.put(@valid_attrs, :user_id, user.id)
      assert {:ok, %Event{} = event1} = Events.create_event(attrs)
      assert {:ok, %Event{} = event2} = Events.create_event(attrs)
    ...
    test "update_event/2 with valid data updates the event name", %{user: user} do
      event = event_fixture(user)
    ...
    test "update_event/2 does not allow changing the slug", %{user: user} do
      event = event_fixture(user)
      original_slug = event.slug
    ...
    test "update_event/2 with invalid data returns error changeset", %{user: user} do
      event = event_fixture(user)
      assert {:error, %Ecto.Changeset{}} = Events.update_event(event, @invalid_attrs)
      assert event.id == Events.get_event!(event.id).id
    end

    test "delete_event/1 deletes the event", %{user: user} do
      event = event_fixture(user)
      assert {:ok, %Event{}} = Events.delete_event(event)
      assert_raise Ecto.NoResultsError, fn -> Events.get_event!(event.id) end
    end

    test "change_event/1 returns an event changeset", %{user: user} do
      event = event_fixture(user)
      assert %Ecto.Changeset{} = Events.change_event(event)
    end
  end

  defp event_fixture(user, attrs \\ %{}) do
    {:ok, event} =
      attrs
